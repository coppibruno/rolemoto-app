...
> **Data:** 2026-09-23  
> **Escopo deste índice:** specs **032–042** (nome do arquivo histórico `032-039-…` mantido para não quebrar links)  
> **Uso:** um cenário de teste / PR por **ID** (não precisa fechar a spec inteira de uma vez).

## Ordem sugerida

1. F2, C1 → 2. E1 → 3. N1 → 4. F1 → 5. A1 → A2 → 6. P1 → P2 → 7. L1 → L2 → 8. G2 → G3 → G1 → 9. **S1** → S3 → S2 → 10. **T1** → 11. **K2** → **K1** → K3 → K4
...
| **042** | [042-push-nativo-capacitor.md](./042-push-nativo-capacitor.md) | K1–K4 | Push FCM nativo no APK + Web Push; `dispositivos.plataforma` |
...
| 042 | Estende **013** (segundo canal: FCM nativo Capacitor; web/PWA intactos) |
