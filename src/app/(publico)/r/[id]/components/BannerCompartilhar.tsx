...
import { COPY_BANNER, COPY_COMPARTILHAR } from "../constants";
...
export const BannerCompartilhar = ({ dados }: Props) => {
  const { compartilhar, feedback } = useCompartilharConvite(dados);

  return (
    <section className={styles.banner}>
      <div className={styles.bannerTopo}>
        <span className={styles.bannerIcone}>
          <IconeWhatsApp />
        </span>
        <p className={styles.bannerTexto}>{COPY_BANNER}</p>
      </div>
      <button
        type="button"
